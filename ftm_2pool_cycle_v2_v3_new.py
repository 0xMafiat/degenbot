...
WEBSOCKET_URI = "ws://127.0.0.1:18546" #TODO: change for node
...
MIN_PROFIT_FTM = int(0.001 * 10**18)
...
async def watch_events():
    ...
    def process_sync_event(message: dict):
        ...
        try:
            ...
            processed_syncs += 1
            if VERBOSE_EVENTS and processed_syncs % 100 == 0:
    ...
    def process_mint_event(message: dict):
        ...
        try:
            ...
            processed_mints += 1
            if VERBOSE_EVENTS and processed_mints % 100 == 0:
    ...
    def process_burn_event(message: dict):
        ...
        try:
            ...
            processed_burns += 1
            if VERBOSE_EVENTS and processed_burns % 100 == 0:
...
async def execute_arb(
    ...
    if arb_profit > MIN_PROFIT_FTM: # and not DRY_RUN:
        ...
        total_priority_fee = int(MINER_TIP * arb_profit) if (int(MINER_TIP * arb_profit) > 0) else (0.03 * 10**18) #TODO change
        ...
        try:
            for transaction in transactions_to_bundle:
                try:
                    simulation = w3.eth.call(transaction)
                except Exception as e:
                    ...
                    if simulation == b'':
                        ...
                        print(f"simulation failed for an unknown reason for transaction: {transaction}")
        except Exception as e:
            ...
            for i in range(bundle_valid_blocks):
                while True:
                    ...
                    try:
                        ...
                        tx_hash = w3.eth.send_raw_transaction(bundle)
                    except Exception as e:
                        print(e)
...
def record_transaction(
    ...
    with open("submitted_transactions.json", "w") as file:
        json.dump(submitted_txs, file, indent=2)
...
for filename in ["fantom_blacklisted_tokens.json"]:
    try:
        with open(filename) as file:
            BLACKLISTED_TOKENS.append(json.load(file))
    except FileNotFoundError:
...
for filename in ["fantom_blacklisted_arbs.json"]:
    try:
        with open(filename) as file:
            BLACKLISTED_ARBS.append(json.load(file))
    except FileNotFoundError:
        with open(filename, "w") as file:
